...
/**
 * A {@code Quaternion} vector defines a complex number in four dimensions.
 * Quaternions are particularly useful in representing 3D space rotations.
 * 
 * @author Waffles
 * @since Apr 30, 2016
 * @version 1.0
 ...
 * @see Vector4
 */
public class Quaternion extends Vector4 implements Algebraic
{
	/**
	...
	public static Matrix3x3 rotate3D(Quaternion q)
	{
		...
		Matrix3x3 m = Matrices.identity(3);
	 ...
	 * 
	 * @param x  a complex imaginary part
	 * @param y  a complex imaginary part
	 * @param z  a complex imaginary part
	 * @param w  a complex real part
	 */
	...
	/**
	 * Returns a {@code Quaternion} angle.
	 * 
	...
	/**
	 * Returns a {@code Quaternion} axis.
	 * 
	...
	public Vector3 Axis()
	{
		return (Vector3) resize(3).normalize();
	}
	...
	@Override
	public Format<Quaternion> Formatter()
	{
		return c ->
		{
			String txt = "";
			...
			txt += (c.W() == 0 ? "" : c.W() < 0 ? " - " : " + ");
			txt += (c.W() == 0 ? "" : Floats.abs(c.W())			);
			txt += (c.X() == 0 ? "" : c.X() < 0 ? " - " : " + ");
			txt += (c.X() == 0 ? "" : Floats.abs(c.X()) + "i"	);
			txt += (c.Y() == 0 ? "" : c.Y() < 0 ? " - " : " + ");
			txt += (c.Y() == 0 ? "" : Floats.abs(c.Y()) + "j"	);
			txt += (c.Z() == 0 ? "" : c.Z() < 0 ? " - " : " + ");
			txt += (c.Z() == 0 ? "" : Floats.abs(c.Z()) + "k");
			txt += (txt.equals("") ? " + " + c.W() : "");
	...
	@Override
	public Quaternion times(Algebraic a)
	{
		return times((Quaternion) a);
	}
