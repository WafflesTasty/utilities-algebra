package waffles.utils.alg.linear.solvers.matrix.exact.types;
...
import waffles.utils.alg.linear.solvers.matrix.exact.LinearSystem;
import waffles.utils.alg.linear.solvers.matrix.factor.LUFactor;
...
{	
	...
	public static interface Hints extends MatrixSolver.Hints
	{
		...
		/**
		 * Checks if a pivot value is nearly zero.
		 * 
		 * @return  {@code true} to check the pivot
		 */
		public default boolean checkPivot()
		{
			return true;
		}
	}
	...
	Matrix backward(Matrix b)
	{
		...
		Matrix x = Matrices.create(c1, c2);
		...
		for(int c = 0; c < c2; c++)
		{
			...
			{	
				...
				float w = 0f;
				for(int k = r + 1; k < c1; k++)
				{
					w += a.get(r, k) * x.get(k, c);
				}
