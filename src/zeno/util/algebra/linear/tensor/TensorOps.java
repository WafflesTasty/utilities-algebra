...
import zeno.util.algebra.linear.tensor.operate.TensorAddition;
import zeno.util.algebra.linear.tensor.operate.TensorDotProduct;
import zeno.util.algebra.linear.tensor.operate.TensorEquality;
 ...
 * 
 *
 * @param <T>  the type of the underlying tensor
 * @see Operator
 * @see Tensor
 */
public interface TensorOps<T extends Tensor> extends Operator<Tensor>
{		
	/**
	...
	@Override
	public abstract T Operable();
	
	@Override
	public default TensorOps<T> instance(Tensor t)
	{
		return () ->
		{
			return (T) t;
	 ...
	 * 
	 * @param tensor  a tensor to compare
	 ...
	 * @see Tensor
	 */
	public default Operation<Boolean> equality(T tensor, int ulps)
	{
		return new TensorEquality(Operable(), tensor, ulps);
	}
		
	/**
	 * Returns a tensor dot product {@code Operation}.
	 * 
	 * @param tensor  a tensor to multiply with
	 ...
	 * @see Float
	 */
	public default Operation<Float> dotproduct(T tensor)
	{
		return new TensorDotProduct(Operable(), tensor);
	}
	
	/**
	 * Returns a tensor sum {@code Operation}.
	 * 
	 * @param tensor  a tensor to add
	 ...
	 * @see Tensor
	 */
	public default Operation<T> addition(T tensor)
	{
		return new TensorAddition<>(Operable(), tensor);
	}
	
	
	/**
	 * Computes a scalar product in the {@code TensorOps}.
	 * 
	 ...
	 * @see Tensor
	 */
	public default T multiply(float val)
	{
